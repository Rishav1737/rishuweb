...
// Real 48-frame scroll-scrub sequence, self-hosted in /public/scroll-frames/.
// Files are named AboutSection_00000.webp ... AboutSection_00047.webp.
function buildImageUrl(index: number): string {
  const padded = String(index).padStart(5, "0");
  return `/scroll-frames/AboutSection_${padded}.webp`;
}

const aboutSectionImages = Array.from({ length: 48 }, (_, i) => ({ index: i }));

// Ratio: 5 units (images) + 2 units (text reveal) = 7 total → h-[700vh]
const IMAGE_DURATION = 5;
const TEXT_DURATION = 2;

// Grayscale accent (light gray, pops on the dark backgrounds used in this section)
const redColor = "oklch(0.92 0 0)"; 
  ...
  return (
    ...
    <div ref={sectionRef} className="relative h-[800vh]">
      {/* Loading State — scoped to this section (absolute, not fixed) so the
          frame preload never covers the hero/above-the-fold content */}
      {!imagesLoaded && (
        ...
        <div ref={textRef} className="pointer-events-none absolute inset-0">
          ...
          <div className="absolute inset-y-0 left-0 flex w-full flex-col justify-center px-6 sm:w-[55%] sm:px-14 lg:px-20">
            <div className="relative max-w-xl p-8 sm:p-10">
              ...
              >
                It's been 2 and a half years of doing motion editing , 
                video editing and I have gained a lot of knowledge about Ae and Pr.
                So yeah, If you want me to create something awesome, just schedule a booking!
              </p>
              ...
              >
                {["GAMING INCLUDED", "MOTION", "ANIMATION", "EDIT"].map(
                  (discipline, i) => (
                    <span
                      key={discipline}
