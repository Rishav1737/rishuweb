...
export default function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const handleMouseOver = (e: MouseEvent) => {
      ...
      // Iframes (YouTube embeds, etc.) are separate documents — once the
      // cursor is over one, this page stops receiving mousemove events
      // entirely, so our custom cursor would freeze in place while the
      // native cursor reappears underneath it. Hide ours the moment we
      // enter one so there's no stuck/duplicate cursor, and let the
      // browser's native cursor take over for that area instead.
      // Add `data-native-cursor` to any non-iframe element (e.g. a custom
      // video player wrapper) that should behave the same way.
      const nativeCursorEl = target.closest("iframe, [data-native-cursor]");
      if (nativeCursorEl) {
        setIsVisible(false);
        return;
      }
